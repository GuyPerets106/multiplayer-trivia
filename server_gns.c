...
void send_multicast_message(int sock, struct sockaddr_in addr, int msg_type, const char *msg_data) {
    ...
    printf("msg type: %d\n", msg.type);
    // Send the message
    int ret = sendto(sock, &msg, sizeof(msg), 0, (struct sockaddr *)&addr, sizeof(addr));
    printf("from multicast send # bytes: %d\n", ret);
}
...
int main() {
    ...
    if((multicast_sock = socket(AF_INET, SOCK_DGRAM, 0)) < 0){ // UDP socket
	perror("UDP FAILED\n");
	exit(EXIT_FAILURE);
    }
    int ttl = 10;
    ...
    if(bind(multicast_sock, (struct sockaddr *)&multicast_addr, sizeof(multicast_addr)) == -1){
